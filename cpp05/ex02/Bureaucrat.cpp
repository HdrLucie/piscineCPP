...
void	Bureaucrat::signForm(AForm &formulaire) const
{
	try {
...
void	Bureaucrat::executeForm(AForm &formulaire) const
{
	try {

		formulaire.execute(*this);
		std::cout << this->getName() << " executed " << formulaire.getName() << std::endl;
	}
	catch (std::exception const &e)
	{
		std::cerr << this->getName() << " couldn't execute " << formulaire.getName() << " because : " << e.what() << std::endl;
	}
	return ;
}
